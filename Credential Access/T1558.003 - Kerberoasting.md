...
// STEP 1: create a list of account to be checked based on their requests during the last <_period> hour.
...
let AnomTargetUserNames = (
	...
	// check if the anomly happened on the current day.
    | where TimeGenerated >= startofday(now())
    // we are looking for positive outliers, meaning increase in the volume.
    // this threashold can be adjusted based on the number of false positives or false negatives
    | where outliers > 1.5
...
//         Avg value will also be used for preventing the rule triggering the same alert again.  
